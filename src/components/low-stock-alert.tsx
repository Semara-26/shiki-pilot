...
export function LowStockAlert({ products, className }: LowStockAlertProps) {
  ...
  if (isSafe) {
    return (
      <div
        className={cn(
          "flex items-center gap-3 rounded-md px-4 py-3 bg-zinc-900/40 border border-emerald-500/20 shadow-sm",
          className,
        )}
      >
  ...
  return (
    <div
      className={cn(
        "flex flex-col gap-3 rounded-md px-4 py-3 bg-amber-950/30 border border-amber-500/50 shadow-sm",
        className,
      )}
    >
      ...
      <div className="flex flex-wrap gap-2">
        {products.map((product) => (
          ...
          >
            <span
              className="font-mono text-[10px] text-amber-200/90 truncate max-w-[150px]"
              title={product.name}
            >
              {product.name}
            </span>
