...
export async function GET(req: Request) {
  try {
    ...
    if (!['daily', 'weekly', 'monthly'].includes(period)) {
      return new Response(JSON.stringify({ error: 'Parameter period tidak valid.' }), {
        status: 400,
    ...
    if (format !== 'csv') {
      return new Response(
        JSON.stringify({ error: 'Format PDF tidak didukung via API. Gunakan halaman Analytics untuk ekspor PDF.' }),
