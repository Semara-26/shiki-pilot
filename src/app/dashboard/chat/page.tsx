...
export default function ChatPage() {
  ...
  /** Optimistic UI: tampilkan pesan user segera sebelum response dari server. */
  ...
  // Clear optimistic message when the real user message appears in the stream/hook state
  useEffect(() => {
  ...
  const showOptimisticUser =
    ...
    (messages.length === 0 || lastMessage?.role !== 'user' || getMessageText(lastMessage.parts) !== pendingUserMessage);
